
<html>
<head>
<meta charset="UTF-8">
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" >
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
</head>
<body>

	
		<ul  class="nav nav-tabs justify-content-center nav-fill" style="background-color: #53e3a6;" >
		<li class="nav-item "><a  class="nav-link text-light" href=/patientSearch/nurse><i class="fa fa-users" aria-hidden="true"></i> Pacijenti</a></li>
		<li class="nav-item "><a class="nav-link text-light" href=/nurse/prescriptions/uncertified><i class="fa fa-file-text" aria-hidden="true"></i> Recepti</a></li>
		<li class="nav-item "><a  class="nav-link text-light" href="/godisnjiOdmorRezervisanjeSestra"><i class="fa fa-sun-o" aria-hidden="true"></i> Odmor </a></li>
		<li class="nav-item "><a class="nav-link text-light" href="/nurseProfile"><i class="fa fa-user" aria-hidden="true"></i> Profil</a></li>
		<li class="nav-item "><a class="nav-link text-light" class="btn btn-outline-success" href="/odjava"><i class="fa fa-sign-out" aria-hidden="true"></i></i> Odjava</a></li>
		</ul>
	

</body>
</html>